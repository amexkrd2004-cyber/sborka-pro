...
#### Пауза — согласование в МойСклад (**активно**)

Заказчик выполняет «домашнюю работу» в МойСклад: финальные **имена статусов**, статус **«в работе»** у сборщика, **доп. поля заказа** под штрихкод. Пока эти данные не переданы в проект, **не фиксируем в коде** реализацию **`PATCH /orders/:id/status`** и кнопки смены статусов в приложении.

**После паузы передать ассистенту / дописать сюда:**

| Что принести | Зачем |
|--------------|--------|
| Точные строки **имён статусов** в МойСклад (как в интерфейсе) и логика **переходов** («из А в Б»). | Настройка API и кнопок. |
| Как называется и как задаётся статус **«в работе»** (имя в МС, связь с **claim** в приложении). | Единая модель «взял заказ ↔ статус в МС». |
| **Доп. поля** заказа для ШК: системные имена / типы (из карточки заказа или экспорта), при необходимости скрин. | Догрузка в **GET /orders** / карточку и сканер. |
